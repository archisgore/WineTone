...
def test_public_route_returns_200(base_url, path):
...
def test_auth_required_endpoint_returns_401(base_url, path, payload, expected):
    ...
    """
    r = httpx.post(f"{base_url}{path}", data=payload, timeout=30,
...
def test_healthz_returns_ok(base_url):
    ...
    r = httpx.get(f"{base_url}/healthz", timeout=15)
    assert r.status_code == 200
    body = r.json()
...
def test_healthz_503s_on_db_failure_design(base_url):
    ...
    body = httpx.get(f"{base_url}/healthz", timeout=15).json()
    assert "db" in body
...
def test_active_tab_highlight(page, base_url, path, label):
...
def test_catalog_freetext_search_returns_results(page, base_url):
...
def test_vocab_search_renders_input(page, base_url):
    page.goto(f"{base_url}/vocab")
    inp = page.locator('input[name="query"]').first
    assert inp.is_visible()


def test_users_directory_renders(page, base_url):
...
def test_wine_detail_page_renders(page, base_url):
...
def test_palate_page_renders(page, base_url):
    """/u/archisgore/palate is the canonical palate URL for the seeded user."""
    page.goto(f"{base_url}/u/archisgore/palate")
...
def test_scanner_page_renders(page, base_url):
    """/wines/scan renders the dropzone (or the 'not configured' message)."""
    page.goto(f"{base_url}/wines/scan")
...
def test_pwa_manifest_serves(base_url):
    """The PWA manifest is reachable + parses as JSON with required fields."""
    r = httpx.get(f"{base_url}/static/manifest.json", timeout=15)
    assert r.status_code == 200
    m = r.json()
...
def test_service_worker_serves(base_url):
    r = httpx.get(f"{base_url}/static/service-worker.js", timeout=15)
    assert r.status_code == 200
...
def test_security_headers_present(base_url):
    r = httpx.head(f"{base_url}/", timeout=15)
    h = r.headers
...
def test_robots_txt(base_url):
    r = httpx.get(f"{base_url}/robots.txt", timeout=15)
    assert r.status_code == 200
    assert "Sitemap:" in r.text


def test_sitemap_xml(base_url):
    r = httpx.get(f"{base_url}/sitemap.xml", timeout=15)
    assert r.status_code == 200
...
def test_admin_reports_404_anonymous(base_url):
    """/admin/reports returns 404 (not 403) to non-admins, so the
    route's existence doesn't leak."""
    r = httpx.get(f"{base_url}/admin/reports", timeout=15,
                  follow_redirects=False)
